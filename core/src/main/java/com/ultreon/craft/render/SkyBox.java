...
public class SkyBox implements Disposable {
    ...
    public SkyBox(Pixmap cubeMap) {
        ...
        for (int x = 0; x < w; x++)
            for (int y = 0; y < h; y++) {
                //-X
                if (x <= w / 4 && y >= h / 3 && y <= h * 2 / 3) data[1].drawPixel(x, y - h / 3, cubeMap.getPixel(x, y));
                //+Y
                if (x >= w / 4 && x <= w / 2 && y <= h / 3) data[2].drawPixel(x - w / 4, y, cubeMap.getPixel(x, y));
                //+Z
    ...
    private void init() {
        vertexShader = new ResourceFileHandle(UltreonCraft.id("shaders/skybox.vert")).readString();
        fragmentShader = new ResourceFileHandle(UltreonCraft.id("shaders/skybox.frag")).readString();
