...
public class AdvancedMeshBuilder extends MeshBuilder {
    ...
    public void begin (final long attributes) {
        this.begin(AdvancedMeshBuilder.createAttributes(attributes), -1);
    }
    ...
    public void begin (final long attributes, int primitiveType) {
        this.begin(AdvancedMeshBuilder.createAttributes(attributes), primitiveType);
    }
    ...
    @Override
    public Mesh end () {
        return this.end(new Mesh(true, Math.min(vertices.size / stride, AdvancedMeshBuilder.MAX_VERTICES), indices.size, attributes));
    }
    ...
    private final static void transformPosition (final float[] values, final int offset, final int size, Matrix4 transform) {
        if (size > 2) {
            AdvancedMeshBuilder.vTmp.set(values[offset], values[offset + 1], values[offset + 2]).mul(transform);
            values[offset] = AdvancedMeshBuilder.vTmp.x;
            values[offset + 1] = AdvancedMeshBuilder.vTmp.y;
            values[offset + 2] = AdvancedMeshBuilder.vTmp.z;
        } else if (size > 1) {
            AdvancedMeshBuilder.vTmp.set(values[offset], values[offset + 1], 0).mul(transform);
            values[offset] = AdvancedMeshBuilder.vTmp.x;
            values[offset + 1] = AdvancedMeshBuilder.vTmp.y;
        } else
            values[offset] = AdvancedMeshBuilder.vTmp.set(values[offset], 0, 0).mul(transform).x;
    }

    private final static void transformNormal (final float[] values, final int offset, final int size, Matrix3 transform) {
        if (size > 2) {
            AdvancedMeshBuilder.vTmp.set(values[offset], values[offset + 1], values[offset + 2]).mul(transform).nor();
            values[offset] = AdvancedMeshBuilder.vTmp.x;
            values[offset + 1] = AdvancedMeshBuilder.vTmp.y;
            values[offset + 2] = AdvancedMeshBuilder.vTmp.z;
        } else if (size > 1) {
            AdvancedMeshBuilder.vTmp.set(values[offset], values[offset + 1], 0).mul(transform).nor();
            values[offset] = AdvancedMeshBuilder.vTmp.x;
            values[offset + 1] = AdvancedMeshBuilder.vTmp.y;
        } else
            values[offset] = AdvancedMeshBuilder.vTmp.set(values[offset], 0, 0).mul(transform).nor().x;
    }
    ...
    private final void addVertex (final float[] values, final int offset) {
        ...
        if (vertexTransformationEnabled) {
            AdvancedMeshBuilder.transformPosition(vertices.items, o + posOffset, posSize, positionTransform);
            if (norOffset >= 0) AdvancedMeshBuilder.transformNormal(vertices.items, o + norOffset, 3, normalTransform);
            if (biNorOffset >= 0) AdvancedMeshBuilder.transformNormal(vertices.items, o + biNorOffset, 3, normalTransform);
            if (tangentOffset >= 0) AdvancedMeshBuilder.transformNormal(vertices.items, o + tangentOffset, 3, normalTransform);
        }
    ...
    public short vertex (Vector3 pos, Vector3 nor, Color col, Vector2 uv) {
        if (vindex > AdvancedMeshBuilder.MAX_INDEX) throw new GdxRuntimeException("Too many vertices used");
    ...
    public short vertex (final VertexInfo info) {
        return this.vertex(info.hasPosition ? info.position : null, info.hasNormal ? info.normal : null,
    ...
    public void line (Vector3 p1, Vector3 p2) {
        this.line(vertTmp1.set(p1, null, null, null), vertTmp2.set(p2, null, null, null));
    }

    @Override
    public void line (float x1, float y1, float z1, float x2, float y2, float z2) {
        this.line(vertTmp1.set(null, null, null, null).setPos(x1, y1, z1), vertTmp2.set(null, null, null, null).setPos(x2, y2, z2));
    }

    @Override
    public void line (Vector3 p1, Color c1, Vector3 p2, Color c2) {
        this.line(vertTmp1.set(p1, null, c1, null), vertTmp2.set(p2, null, c2, null));
    }
    ...
    public void triangle (Vector3 p1, Vector3 p2, Vector3 p3) {
        this.triangle(vertTmp1.set(p1, null, null, null), vertTmp2.set(p2, null, null, null), vertTmp3.set(p3, null, null, null));
    }

    @Override
    public void triangle (Vector3 p1, Color c1, Vector3 p2, Color c2, Vector3 p3, Color c3) {
        this.triangle(vertTmp1.set(p1, null, c1, null), vertTmp2.set(p2, null, c2, null), vertTmp3.set(p3, null, c3, null));
    }
    ...
    public void rect (Vector3 corner00, Vector3 corner10, Vector3 corner11, Vector3 corner01, Vector3 normal) {
        this.rect(vertTmp1.set(corner00, normal, null, null).setUV(0f, 1f), vertTmp2.set(corner10, normal, null, null).setUV(1f, 1f),
    ...
    public void rect (float x00, float y00, float z00, float x10, float y10, float z10, float x11, float y11, float z11, float x01,
        ...
        this.rect(vertTmp1.set(null, null, null, null).setPos(x00, y00, z00).setNor(normalX, normalY, normalZ).setUV(0f, 1f),
    ...
    public void addMesh (Mesh mesh, int indexOffset, int numIndices) {
        ...
        AdvancedMeshBuilder.tmpVertices.clear();
        AdvancedMeshBuilder.tmpVertices.ensureCapacity(numFloats);
        AdvancedMeshBuilder.tmpVertices.size = numFloats;
        mesh.getVertices(AdvancedMeshBuilder.tmpVertices.items);

        AdvancedMeshBuilder.tmpIndices.clear();
        AdvancedMeshBuilder.tmpIndices.ensureCapacity(numIndices);
        AdvancedMeshBuilder.tmpIndices.size = numIndices;
        mesh.getIndices(indexOffset, numIndices, AdvancedMeshBuilder.tmpIndices.items, 0);

        this.addMesh(AdvancedMeshBuilder.tmpVertices.items, AdvancedMeshBuilder.tmpIndices.items, 0, numIndices);
    }
    ...
    public void addMesh (float[] vertices, short[] indices, int indexOffset, int numIndices) {
        if (AdvancedMeshBuilder.indicesMap == null)
            AdvancedMeshBuilder.indicesMap = new IntIntMap(numIndices);
        else {
            AdvancedMeshBuilder.indicesMap.clear();
            AdvancedMeshBuilder.indicesMap.ensureCapacity(numIndices);
        }
        this.ensureIndices(numIndices);
        ...
        for (int i = 0; i < numIndices; i++) {
            ...
            int didx = AdvancedMeshBuilder.indicesMap.get(sidx, -1);
            if (didx < 0) {
                this.addVertex(vertices, sidx * stride);
                AdvancedMeshBuilder.indicesMap.put(sidx, didx = lastIndex);
            }
    ...
    public void addMesh (float[] vertices, short[] indices) {
        ...
        for (int i = 0; i < indices.length; ++i)
            this.index((short)((indices[i] & 0xFFFF) + offset));
    }
    ...
    public void circle (float radius, int divisions, final Vector3 center, final Vector3 normal, final Vector3 tangent,
        ...
        this.circle(radius, divisions, center.x, center.y, center.z, normal.x, normal.y, normal.z, tangent.x, tangent.y, tangent.z,
