...
public class TextObjectRenderer {
    ...
    public void render(Renderer renderer, Color altColor, float x, float y, boolean shadow) {
        ...
        for (TextObject cur : this.text) {
            ...
            this.renderSingle(renderer, y, shadow, rawText, color, bold, italic, underlined, strikethrough);
        }
    }

    private void renderSingle(Renderer renderer, float y, boolean shadow, String rawText, Color color, boolean bold, boolean italic, boolean underlined, boolean strikethrough) {
        float scale = 1;
    ...
    private void renderPart(Renderer renderer, float y, boolean shadow, Color color, boolean bold, boolean italic, boolean underlined, boolean strikethrough, String part, float scale) {
        if (this.currentFont == Font.UNIFONT)
            this.font.drawTextScaled(renderer, this.currentFont, renderer.getBatch(), part, this.currentX, y + (this.font.bitmapFont.getLineHeight() - Font.UNIFONT.getLineHeight() * 0.5F) / 2, bold, italic, underlined, strikethrough, scale, color, shadow);
        else
