 ...
 * <p>
 * Begins at the origin, the steps one block at a time horizontal.
 * Keep going horizontal until the next block reached
 ...
 * or one of the blocks in the next row is dirty.
 * When an incompatible row is found, the marching stops, and a rectangle is completed.
 * All the blocks in the completed rectangle are marked as dirty, and the rectangle is used as a face.
 * This process is repeated with the origin of the next non-dirty block
 * until there are no more dirty blocks on the face.
 * <p>
 * Example:
 * <p>
 * Source
...
public class GreedyMesher implements Mesher {
    ...
    public List<Face> getFaces(UseCondition condition, OccludeCondition ocCond, MergeCondition shouldMerge) {
        ...
        final int width = World.CHUNK_SIZE;
        final int depth = World.CHUNK_SIZE;
        final int height = World.CHUNK_SIZE;
        // Top, bottom
            ...
            for (int z = 0; z < depth; z++) {
                for (int x = 0; x < width; x++) {
                    try {
                        ...
                        ClientChunk topNeighborChunk = this.chunk;
                        ClientChunk bottomNeighborChunk = this.chunk;
                        if (topNeighborY >= World.CHUNK_SIZE) {
                            topNeighborChunk = this.chunk.getWorld().getChunk(this.chunk.getPos().x(), this.chunk.getPos().y() + 1, this.chunk.getPos().z());
                            topNeighborY -= World.CHUNK_SIZE;
                        }
                        if (bottomNeighborY < 0) {
                            bottomNeighborChunk = this.chunk.getWorld().getChunk(this.chunk.getPos().x(), this.chunk.getPos().y() - 1, this.chunk.getPos().z());
                            bottomNeighborY += World.CHUNK_SIZE;
                        }
                        if (topNeighborChunk != null) {
                            Block topNeighborBlk = topNeighborChunk.get(x, topNeighborY, z);
                            if (!ocCond.shouldOcclude(curBlock, topNeighborBlk)) {
                                topMask[x][z] = true;
                        ...
                        if (bottomNeighborChunk != null) {
                            Block bottomNeighborBlk = bottomNeighborChunk.get(x, bottomNeighborY, z);
                            if (!ocCond.shouldOcclude(curBlock, bottomNeighborBlk)) {
                                btmMask[x][z] = true;
            ...
            for (int y = 0; y < height; y++) {
                for (int z = 0; z < depth; z++) {
                    try {
                        ...
                        if (westNeighborX < 0) {
                            westNeighborChunk = this.chunk.getWorld().getChunk(this.chunk.getPos().x() - 1, this.chunk.getPos().y(), this.chunk.getPos().z());
                            westNeighborX += World.CHUNK_SIZE;
                        }
                        if (eastNeighborX >= World.CHUNK_SIZE) {
                            eastNeighborChunk = this.chunk.getWorld().getChunk(this.chunk.getPos().x() + 1, this.chunk.getPos().y(), this.chunk.getPos().z());
                ...
                for (int x = 0; x < width; x++) {
                    try {
                        ...
                        if (northNeighborZ >= World.CHUNK_SIZE) {
                            northNeighborChunk = this.chunk.getWorld().getChunk(this.chunk.getPos().x(), this.chunk.getPos().y(), this.chunk.getPos().z() + 1);
                            northNeighborZ -= World.CHUNK_SIZE;
                        } else if (southNeighborZ < 0) {
                            southNeighborChunk = this.chunk.getWorld().getChunk(this.chunk.getPos().x(), this.chunk.getPos().y(), this.chunk.getPos().z() - 1);
     ...
     */
    private void greedy(List<Face> outputList, BlockFace side, MergeCondition mergeCond, boolean[][] mask, PerCornerLightData[][] lightPCLDs, int z, int offsetX, int offsetY, int offsetZ) {
        ...
        try {
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    ...
                    outputList.add(new Face(side, block, ll, x + offsetX, y + offsetY, endX + offsetX, endY + offsetY, z + offsetZ, 1));
                }
            }
        } catch (PosOutOfBoundsException ex) {
    ...
    /**
     * Averages light values in a corner.
     *
        ...
        for (int sy = y + negY; sy <= y + posY; sy++) {
            for (int sz = cz + negZ; sz <= cz + posZ; sz++) {
                for (int sx = cx + negX; sx <= cx + posX; sx++) {
                    ...
                    if (sChunk.isOutOfBounds(fixedSx, sy, fixedSz)) continue;
    ...
    private boolean shouldMerge(Block id1, float light1, PerCornerLightData lightData1, Block id2, float light2, PerCornerLightData lightData2) {
        if (id2 == null || id1 == null) return false;
        if (lightData1 == null || lightData2 == null) return false;
    ...
    public static class Face {
        ...
        /**
         * @param sunlightLevel Sunlight level.
         */
        public Face(BlockFace side, Block block, float lightLevel, int startX, int startY, int endX, int endY, int z, float sunlightLevel) {
    ...
    private int realX(BlockFace side, int x, int z) {
        return switch (side) {
    ...
    private int realY(BlockFace side, int y, int z) {
        return switch (side) {
    ...
    private @Nullable Block block(ClientChunk chunk, int x, int y, int z) {
        ...
        this.tmp3i.set(chunk.getPos().x(), chunk.getPos().y(), chunk.getPos().z()).mul(16).add(x, y, z);
        ClientChunk chunkAt = world.getChunkAt(this.tmp3i.x, this.tmp3i.y, this.tmp3i.z);
        if (chunkAt != null)
            return chunkAt.get(World.localizeBlock(this.tmp3i.x, this.tmp3i.y, this.tmp3i.z, this.tmp3i));
        return Blocks.AIR;
    }
    ...
    private int blockLight(ClientChunk chunk, int x, int y, int z) {
        ...
        ClientChunk chunkAt = world.getChunkAt(this.tmp3i.x, this.tmp3i.y, this.tmp3i.z);
        if (chunkAt != null)
            return chunkAt.getBlockLight(World.localizeBlock(this.tmp3i.x, this.tmp3i.y, this.tmp3i.z, this.tmp3i));
        return 0;
    }
    ...
    private int sunlight(ClientChunk chunk, int x, int y, int z) {
        ...
        ClientChunk chunkAt = world.getChunkAt(this.tmp3i.x, this.tmp3i.y, this.tmp3i.z);
        if (chunkAt != null)
            return chunkAt.getSunlight(World.localizeBlock(this.tmp3i.x, this.tmp3i.y, this.tmp3i.z, this.tmp3i));
        return 0;
    }
}