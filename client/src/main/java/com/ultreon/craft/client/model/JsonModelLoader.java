...
public class JsonModelLoader {
    ...
    public JsonModel load(Block block) throws IOException {
        Identifier elementID = block.getId().mapPath(path -> "models/blocks/" + path + ".json");
    ...
    public JsonModel load(Item item) throws IOException {
        Identifier elementID = item.getId().mapPath(path -> "models/items/" + path + ".json");
        Resource resource = this.resourceManager.getResource(elementID);
        if (resource == null)
            return null;
...
//        GridPoint2 textureSize = loadVec2i(root.getAsJsonArray("texture_size"), new GridPoint2(16, 16));
    ...
    private Map<CubicDirection, FaceElement> loadFaces(JsonObject faces, int textureWidth, int textureHeight) {
        ...
        for (Map.Entry<String, JsonElement> face : faces.entrySet()) {
            CubicDirection blockFace = CubicDirection.valueOf(face.getKey().toUpperCase(Locale.ROOT));
    ...
    public record ModelElement(Map<CubicDirection, FaceElement> blockFaceFaceElementMap, boolean shade,
        ...
        public void bake(int idx, ModelBuilder modelBuilder, Map<String, Identifier> textureElements) {
            ...
            for (Map.Entry<CubicDirection, FaceElement> entry : blockFaceFaceElementMap.entrySet()) {
                CubicDirection blockFace = entry.getKey();
