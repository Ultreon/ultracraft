...
public class BoundingBox implements Serializable {
    ...
    public BoundingBox updateByDelta(double deltaX, double deltaY, double deltaZ) {
        ...
        if (deltaX < 0) this.min.x += deltaX;
        else this.max.x += deltaX;

        if (deltaY < 0) this.min.y += deltaY;
        else this.max.y += deltaY;

        if (deltaZ < 0) this.min.z += deltaZ;
        else this.max.z += deltaZ;
